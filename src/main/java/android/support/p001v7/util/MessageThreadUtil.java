...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C00411();
            ...
            class C00411 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem msg = C00421.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C00431();
            ...
            class C00431 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C00442.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C00442.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C00442.this.mQueue.removeMessages(2);
                                    C00442.this.mQueue.removeMessages(3);
                        ...
                        C00442.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
