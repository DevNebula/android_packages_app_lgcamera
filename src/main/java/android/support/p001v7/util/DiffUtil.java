...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new C00391();

    /* renamed from: android.support.v7.util.DiffUtil$1 */
    static class C00391 implements Comparator<Snake> {
        C00391() {
        }

        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f1x - o2.f1x;
            return cmpX == 0 ? o1.f2y - o2.f2y : cmpX;
        }
    }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f1x != 0 || firstSnake.f2y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f1x + snake.size;
                int endY = snake.f2y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f1x + j;
                    int newItemPos = snake.f2y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f1x + snakeSize;
                int endY = snake.f2y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f1x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f1x + i, 1, this.mCallback.getChangePayload(snake.f1x + i, snake.f2y + i));
                    }
                }
                posOld = snake.f1x;
                posNew = snake.f2y;
                ...
                for (int i = count - 1; i >= 0; i--) {
                    ...
                    switch (status) {
                        ...
                        case 8:
                            ...
                            updateCallback.onMoved(DiffResult.removePostponedUpdate(postponedUpdates, pos, true).currentPos, start);
                            if (status != 4) {
                                break;
                            }
                ...
                for (int i = count - 1; i >= 0; i--) {
                    ...
                    switch (status) {
                        ...
                        case 8:
                            ...
                            PostponedUpdate update2 = DiffResult.removePostponedUpdate(postponedUpdates, pos, false);
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f1x += range.oldListStart;
                snake.f2y += range.newListStart;
                ...
                if (snake.reverse) {
                    left.oldListEnd = snake.f1x;
                    left.newListEnd = snake.f2y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f1x - 1;
                    left.newListEnd = snake.f2y;
                } else {
                    left.oldListEnd = snake.f1x;
                    left.newListEnd = snake.f2y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f1x + snake.size;
                    right.newListStart = snake.f2y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = (snake.f1x + snake.size) + 1;
                    right.newListStart = snake.f2y + snake.size;
                } else {
                    right.oldListStart = snake.f1x + snake.size;
                    right.newListStart = (snake.f2y + snake.size) + 1;
                }
                stack.add(right);
            } else {
                rangePool.add(range);
        ...
        while (d <= dLimit) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake = new Snake();
                    outSnake.f1x = backward[kOffset + k];
                    outSnake.f2y = outSnake.f1x - k;
            ...
            while (k <= d) {
                ...
                if (checkInFwd || k + delta < (-d) || k + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake = new Snake();
                    outSnake.f1x = backward[kOffset + backwardK];
                    outSnake.f2y = outSnake.f1x - backwardK;
