...
class PositionMap<E> implements Cloneable {
    ...
    public void put(int key, E value) {
        ...
        if (i >= this.mSize || this.mValues[i] != DELETED) {
            ...
            if (this.mSize >= this.mKeys.length) {
                int n = PositionMap.idealIntArraySize(this.mSize + 1);
                int[] nkeys = new int[n];
    ...
    public void append(int key, E value) {
        if (this.mSize == 0 || key > this.mKeys[this.mSize - 1]) {
            ...
            if (pos >= this.mKeys.length) {
                int n = PositionMap.idealIntArraySize(pos + 1);
                int[] nkeys = new int[n];
                Object[] nvalues = new Object[n];
                System.arraycopy(this.mKeys, 0, nkeys, 0, this.mKeys.length);
                System.arraycopy(this.mValues, 0, nvalues, 0, this.mValues.length);
    ...
    static int idealBooleanArraySize(int need) {
        return PositionMap.idealByteArraySize(need);
    }

    static int idealShortArraySize(int need) {
        return PositionMap.idealByteArraySize(need * 2) / 2;
    }
    ...
    static int idealLongArraySize(int need) {
        return PositionMap.idealByteArraySize(need * 8) / 8;
    }
}
