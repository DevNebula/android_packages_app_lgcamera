...
final class ZipUtil {
    ...
    static long getZipCrc(File apk) throws IOException {
        RandomAccessFile raf = new RandomAccessFile(apk, LdbConstants.LDB_CAMERA_ID_REAR_NORMAL);
        try {
    ...
    static CentralDirectory findCentralDirectory(RandomAccessFile raf) throws IOException, ZipException {
        ...
        do {
            ...
            if (raf.readInt() == endSig) {
                ...
                dir.size = ((long) Integer.reverseBytes(raf.readInt())) & CameraConstants.MEDIA_RECORDING_MAX_LIMIT;
                dir.offset = ((long) Integer.reverseBytes(raf.readInt())) & CameraConstants.MEDIA_RECORDING_MAX_LIMIT;
                return dir;
            }
            scanOffset--;
