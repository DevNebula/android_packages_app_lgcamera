...
final class BackStackRecord extends FragmentTransaction implements BackStackEntry, OpGenerator {
    ...
    String mName;
    ArrayList<C0001Op> mOps = new ArrayList();
    int mPopEnterAnim;
    int mPopExitAnim;
    ...
    public void dump(String prefix, PrintWriter writer, boolean full) {
        ...
        if (!this.mOps.isEmpty()) {
            ...
            for (int opNum = 0; opNum < numOps; opNum++) {
                String cmdStr;
                C0001Op op = (C0001Op) this.mOps.get(opNum);
                switch (op.cmd) {
                    case 0:
                        cmdStr = "NULL";
                        break;
    ...
    boolean interactsWith(int containerId) {
        int numOps = this.mOps.size();
        for (int opNum = 0; opNum < numOps; opNum++) {
            if (((C0001Op) this.mOps.get(opNum)).fragment.mContainerId == containerId) {
                return true;
            }
        }
        return false;
    ...
    boolean interactsWith(ArrayList<BackStackRecord> records, int startIndex, int endIndex) {
        ...
        int lastContainer = -1;
        for (int opNum = 0; opNum < numOps; opNum++) {
            int container = ((C0001Op) this.mOps.get(opNum)).fragment.mContainerId;
            if (!(container == 0 || container == lastContainer)) {
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    for (int thoseOpIndex = 0; thoseOpIndex < numThoseOps; thoseOpIndex++) {
                        if (((C0001Op) record.mOps.get(thoseOpIndex)).fragment.mContainerId == container) {
                            return true;
                        }
                    }
                }
    ...
    boolean isPostponed() {
        for (int opNum = 0; opNum < this.mOps.size(); opNum++) {
            if (BackStackRecord.isFragmentPostponed((C0001Op) this.mOps.get(opNum))) {
                return true;
            }
        }
        return false;
    ...
    private static boolean isFragmentPostponed(C0001Op op) {
        Fragment fragment = op.fragment;
