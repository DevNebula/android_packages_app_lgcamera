...
class ExifOutputStream extends FilterOutputStream {
    ...
    protected ExifOutputStream(OutputStream ou, ExifBridge iRef) {
    ...
    private void writeExifData() throws IOException {
        if (this.mExifData != null) {
            ...
            this.mInterface.createRequiredIfdAndTag(this.mExifData);
            int exifSize = this.mInterface.calculateAllOffset(this.mExifData);
            if (exifSize + 8 > 65535) {
    ...
    private ArrayList<ExifTag> stripNullValueTags(ExifData data) {
        ...
        if (!(data == null || this.mInterface == null)) {
            List<ExifTag> allTags = data.getAllTags();
            if (allTags != null) {
                for (ExifTag t : allTags) {
                    if (!(t == null || t.getValue() != null || this.mInterface.isExifOffsetTag(t.getTagId()))) {
    ...
    private void writeAllTags(OrderedDataOutputStream dataOutputStream) throws IOException {
        ...
        if (ifd0 != null) {
            writeIfd(ifd0, dataOutputStream);
        }
        IfdData ifdExif = this.mExifData.getIfdData(2);
        if (ifdExif != null) {
            writeIfd(ifdExif, dataOutputStream);
        }
        ...
        if (ifd1 != null) {
            writeIfd(ifd1, dataOutputStream);
        }
    }
