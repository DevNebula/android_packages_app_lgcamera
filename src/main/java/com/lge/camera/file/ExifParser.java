...
class ExifParser {
    ...
    private static final int XMP_ADOBE_POST = 1868719406;
    private static final int XMP_ADOBE_PRE = 1932419428;
    private static final int XMP_DOTCOM = 1668246831;
    private static final int XMP_HTTP_START = 1752462448;
    private static final int XMP_SLASH = 976170862;
    ...
    private ExifParser(InputStream inputStream, int options, ExifBridge iRef) throws IOException, ExifInvalidFormatException {
        if (inputStream == null) {
            throw new IOException("Null argument inputStream to ExifParser");
        }
        this.mInterface = iRef;
        this.mContainExifData = seekTiffData(new CountedDataInputStream(inputStream));
    ...
    protected static ExifParser parse(InputStream inputStream, int options, ExifBridge iRef) throws IOException, ExifInvalidFormatException {
        return new ExifParser(inputStream, options, iRef);
    }

    protected static ExifParser parse(InputStream inputStream, ExifBridge iRef) throws IOException, ExifInvalidFormatException {
    ...
    private ExifTag readTag() throws IOException, ExifInvalidFormatException {
        ...
        } else if (ExifTagBase.isValidType(dataFormat)) {
    ...
    private void checkOffsetOrImageTag(ExifTag tag) {
        if (this.mInterface != null && tag.getComponentCount() != 0) {
            short tid = tag.getTagId();
            int ifd = tag.getIfd();
            try {
                if (this.mInterface.checkAllowedIfd(tid, 0, ifd)) {
                    if (isIfdRequested(2) || isIfdRequested(3)) {
                        registerIfd(2, tag.getValueAt(0));
                    }
                } else if (this.mInterface.checkAllowedIfd(tid, 1, ifd)) {
                    if (isIfdRequested(4)) {
                        registerIfd(4, tag.getValueAt(0));
                    }
                } else if (this.mInterface.checkAllowedIfd(tid, 2, ifd)) {
                    if (isIfdRequested(3)) {
                        registerIfd(3, tag.getValueAt(0));
                    }
                } else if (this.mInterface.checkAllowedIfd(tid, 3, ifd)) {
                    if (isThumbnailRequested()) {
                        registerCompressedImage(tag.getValueAt(0));
                    }
                } else if (this.mInterface.checkAllowedIfd(tid, 4, ifd)) {
                    if (isThumbnailRequested()) {
                        this.mJpegSizeTag = tag;
                    }
                } else if (this.mInterface.checkAllowedIfd(tid, 5, ifd)) {
                    if (!isThumbnailRequested()) {
                        return;
                    }
                    if (tag.hasValue()) {
                ...
                } else if (this.mInterface.checkAllowedIfd(tid, 6, ifd) && isThumbnailRequested() && tag.hasValue()) {
                    this.mStripSizeTag = tag;
                }
            } catch (Exception e) {
                Log.e(TAG, "Exception occured while reading tag!");
            }
        }
    }
    ...
    private boolean seekTiffData(CountedDataInputStream dataStream) throws IOException, ExifInvalidFormatException {
        if (dataStream.readShort() != (short) -40) {
            throw new ExifInvalidFormatException("Invalid JPEG format");
        }
        short marker = dataStream.readShort();
        int[] checkAddr = new int[]{XMP_HTTP_START, XMP_SLASH, XMP_ADOBE_PRE, XMP_ADOBE_POST, XMP_DOTCOM};
        while (marker != (short) -39 && !JpegHeader.isSofMarker(marker)) {
            int length = dataStream.readUnsignedShort();
            if (marker == (short) -31 && length >= 8) {
                int header = dataStream.readInt();
                if (header == XMP_HTTP_START) {
                    int idx = 1;
                    int i = 1;
                    while (i < checkAddr.length && checkAddr[i] == dataStream.readInt()) {
                        idx++;
                        i++;
                    }
                    if (idx == checkAddr.length) {
                        long needSkip = (long) (length - 22);
                        while (needSkip > 0) {
                            needSkip -= dataStream.skip(Math.min(PlaybackStateCompat.ACTION_PLAY_FROM_SEARCH, needSkip));
                        }
    ...
    protected long readUnsignedLong() throws IOException {
        return ((long) readLong()) & CameraConstants.MEDIA_RECORDING_MAX_LIMIT;
    }
