...
class IfdData {
    ...
    private ExifBridge mInterface;
    private int mOffsetToNextIfd = 0;

    IfdData(int ifdId, ExifBridge iRef) {
        this.mIfdId = ifdId;
        ...
        if (obj instanceof IfdData) {
            ...
            if (data.getId() == this.mIfdId && data.getTagCount() == getTagCount()) {
                for (ExifTag tag : data.getAllTags()) {
                    if (this.mInterface != null && !this.mInterface.isExifOffsetTag(tag.getTagId()) && !tag.equals((ExifTag) this.mExifTags.get(Short.valueOf(tag.getTagId())))) {
                        return false;
                    }
                }
                return true;
