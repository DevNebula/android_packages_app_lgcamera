...
class ExifData {
    ...
    private ExifBridge mInterfaec;
    private ArrayList<byte[]> mStripBytes = new ArrayList();
    private byte[] mThumbnail;

    ExifData(ByteOrder order, ExifBridge iRef) {
        this.mByteOrder = order;
        this.mInterfaec = iRef;
    }
    ...
    protected IfdData getOrCreateIfdData(int ifdId) {
        ...
        ifdData = new IfdData(ifdId, this.mInterfaec);
    ...
    protected ExifTag addTag(ExifTag tag, int ifdId) {
        if (tag == null || !ExifTagBase.isValidIfd(ifdId)) {
            return null;
        }
    ...
    protected String getUserComment(short tagId) {
