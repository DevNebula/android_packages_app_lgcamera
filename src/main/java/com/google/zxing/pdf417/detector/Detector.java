...
public final class Detector {
    ...
    private static int[] findGuardPattern(BitMatrix matrix, int column, int row, int width, boolean whiteFirst, int[] pattern, int[] counters) {
        ...
        for (x = patternStart; x < width; x++) {
            if ((matrix.get(x, row) ^ isWhite) != 0) {
                ...
                if (counterPosition != patternLength - 1) {
                    counterPosition++;
                } else if (patternMatchVariance(counters, pattern, 0.8f) < MAX_AVG_VARIANCE) {
        ...
        if (counterPosition != patternLength - 1 || patternMatchVariance(counters, pattern, 0.8f) >= MAX_AVG_VARIANCE) {
            return null;
        }
